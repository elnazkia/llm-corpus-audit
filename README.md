...
├── scripts/          # Runnable Python scripts for demos or specific tasks
│   └── run_demo.py   # Demo script for core functionalities
│   └── generate_llm_outputs.py # Script to generate LLM outputs
├── src/
...
|                                           | Outputs from LLMs based on designed prompts (CSV/JSON)                         | Generated in project                                                          | Partially Implemented (see `scripts/generate_llm_outputs.py` and `data/raw/llm_outputs/initial_llm_outputs.json`) |
...
*   `scripts/generate_llm_outputs.py`: Generates a small set of LLM outputs using pre-defined prompts and a Hugging Face model, saving them to `data/raw/llm_outputs/`.

A demonstration of how to use these modules can be found in `scripts/run_demo.py`.
...
7.  **Run the Demo Script:**
    ...
    *   First, ensure you have some LLM outputs (if you haven't run it yet):
        ```bash
        python scripts/generate_llm_outputs.py
        ```
    *   Then, execute the demo analysis script using Python:
        ```bash
        python scripts/run_demo.py
        ```
    *   This will print analysis results to the console and save generated plots to an `output/` directory (which will be created if it doesn't exist).
...
| 2        | Prompt design & LLM output collection (≥4 registers, ≥3 demo. templates)  | `data/raw/llm_outputs/initial_llm_outputs.json` (initial set); `scripts/generate_llm_outputs.py` |
...
1.  **Prompt engineering:** Design ~20 prompt templates across registers (academic abstract, news lead, Reddit post, email, etc.) and demographic templates ("The <profession> is…", "A <nationality> person…"). (Initial simple prompts in `scripts/generate_llm_outputs.py`)
2.  **Generation:** Use `transformers` pipeline (Llama-2) and/or OpenAI API to sample ~5 completions per prompt → ~200 outputs. (`scripts/generate_llm_outputs.py` provides a basic framework using `gpt2`).
3.  Save to `data/raw/llm_outputs/` (e.g., as CSV or JSON) with metadata: `prompt_id`, `prompt_text`, `register`, `demographic_var`, `model_name`, `output_text`, `generation_date`. (`initial_llm_outputs.json` is the first example).
...
3.  **Analysis and Visualization script** (`scripts/run_demo.py` and subsequent analysis scripts): Use `src/visualization/visualization.py` for plotting. Conduct PCA / clustering to visualise how LLM outputs align with human registers.
...
- [ ] Python scripts for demonstration and analysis (e.g., `scripts/run_demo.py`, plus others for data processing, style analysis, bias audit, annotation).
